...
- **Category**: pick from `categories.md` in this skill directory — and also from `categories.local.md` there if it exists (personal additions, gitignored). Read both before choosing. Emit the full path with ` > ` (e.g. `Food & Dining > Restaurants`, `Housing > Loan > Interest`); a bare `Main` with no sub is fine. Anchor examples: AI subscriptions (Claude, ChatGPT) → `Digital > AI`; streaming (Netflix, YouTube Premium, Prime Video) → `Digital > Streaming Services`; app-store/software → `Digital > Apps` or `Office > Software`; foreign-transaction fees → `Banking > Service Fees`; taxi/rideshare (55688, Uber) → `Transportation > Rideshare & Taxi`; card annual fee → `Credit Cards > Annual Fees`. When the item is unknown, categorize by merchant type — marketplaces (Shopee, Taobao, Amazon…) and department stores are `Shopping`. Reserve `Other > Uncategorized` only for merchants you cannot identify at all.
